...
import static com.msxf.downloader.Utils.HTTP_OK;
import static com.msxf.downloader.Utils.HTTP_PARTIAL;
  ...
  private void executeDownload(DownloadRequest request) {
    ...
    try {
      ...
      if (statusCode != HTTP_OK && statusCode != HTTP_PARTIAL) {
        ...
        throw new DownloadException(statusCode, "Download fail with error http code");
      }
      ...
      if (is != null) {
        ...
        throw new DownloadException(statusCode, "Input byte stream error");
      }
    } catch (IOException e) {
